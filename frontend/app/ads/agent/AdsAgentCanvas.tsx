...
export default function AdsAgentCanvas({ messages }: { messages: any[]|null }){
  const data = useMemo(()=>{
    ...
    const steps = [
      ...
      { key:'angles', title:'Generate Angles', done: !!angles, summary: summarizeAngles(angles) },
      { key:'td', title:'Title & Description', done: !!titleDesc, summary: summarizeTitleDesc(titleDesc) },
      { key:'lc', title:'Landing Copy', done: !!landingCopy, summary: summarizeLandingCopy(landingCopy) },
    ]
    return { steps }
  },[messages])
    ...
    <div className="border rounded p-3 bg-white mb-3">
      ...
      <svg width={totalW} height={totalH} className="block">
        ...
        {data.steps.map((s, i)=>{
          ...
          const color = s.done? '#0f172a' : '#CBD5E1'
          const fill = s.done? '#F8FAFC' : '#FFFFFF'
          return (
            <g key={s.key} className="fadeIn" style={{ animationDelay: `${i*120}ms` }}>
              {/* connector */}
              {i < data.steps.length-1 ? (
                <line x1={padding+nodeW/2} y1={y+nodeH} x2={padding+nodeW/2} y2={nextY} stroke="#94A3B8" strokeWidth={2} className="dash" />
              ) : null}
              {/* node */}
              <rect x={x} y={y} rx={10} ry={10} width={nodeW} height={nodeH} fill={fill} stroke={color} strokeWidth={2} />
              <circle cx={x+16} cy={y+20} r={6} fill={s.done? '#16A34A':'#94A3B8'} />
              {/* title */}
              <text x={x+32} y={y+24} fill={color} fontSize={12} fontWeight={600}>{s.title}</text>
              {/* summary */}
              <text x={x+16} y={y+44} fill="#475569" fontSize={11}>
