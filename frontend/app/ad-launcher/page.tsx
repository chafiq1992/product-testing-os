...
export default function AdLauncherPage(){
  ...
  async function launch(){
    ...
    try{
      ...
      setError(String(err?.response?.data?.detail||err?.message||err))
      try{const latest=await getAdLauncherJob(store,jobId);if(latest.data)setJob(latest.data)}catch{}
    }
    finally{setBusy('')}
  }
  ...
  const savedMetaDraft=job?.status==='launch_failed'&&job?.result?.meta?.campaign_id?job.result.meta:null
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-800">
    ...
    <main className="mx-auto max-w-[1500px] space-y-6 px-4 py-6 md:px-7">
      ...
      {job&&<Card className="overflow-hidden">
        ...
        {savedMetaDraft&&<div className="mx-5 mb-5 rounded-2xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-900"><div className="font-bold">PAUSED campaign saved in Meta Ads Manager</div><p className="mt-2">Campaign <b>{savedMetaDraft.campaign_name||plan?.campaign_name}</b> · ID <b>{savedMetaDraft.campaign_id}</b> is non-delivering and available for manual completion.</p><div className="mt-3 grid gap-2 md:grid-cols-2">{savedMetaDraft.adsets?.filter((item:any)=>item.adset_id).map((item:any)=><div key={item.adset_id} className="rounded-xl border border-blue-200 bg-white p-3 text-xs"><div className="font-bold">{item.adset_name} · ${Number(item.daily_budget_usd).toFixed(2)}</div><div className="mt-2 flex flex-wrap gap-2"><span className={`rounded-full px-2 py-1 ${item.creative_id?'bg-emerald-100 text-emerald-800':'bg-amber-100 text-amber-800'}`}>{item.creative_id?'Creative ready':'Creative missing'}</span><span className={`rounded-full px-2 py-1 ${item.ad_id?'bg-emerald-100 text-emerald-800':'bg-amber-100 text-amber-800'}`}>{item.ad_id?'Ad ready':'Ad missing'}</span></div></div>)}</div><a href={`https://adsmanager.facebook.com/adsmanager/manage/campaigns?act=${encodeURIComponent(String(job?.request?.meta_ad_account_id||adAccountId))}&selected_campaign_ids=${encodeURIComponent(String(savedMetaDraft.campaign_id))}`} target="_blank" rel="noreferrer" className="mt-3 inline-flex rounded-xl bg-blue-700 px-3 py-2 text-xs font-bold text-white">Open saved campaign in Ads Manager</a></div>}
        {metaAppModeError&&<div className="mx-5 mb-5 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><div className="font-bold">Meta app action required before retrying</div><ol className="mt-2 list-decimal space-y-1 pl-5"><li>Open <a href="https://developers.facebook.com/apps/" target="_blank" className="font-semibold underline">Meta for Developers</a> and select the app that issued this token.</li><li>Switch App Mode from Development to Live/Public and confirm the required Marketing API permissions.</li><li>Generate a fresh token from that Live app, update the launcher token, then use Retry Meta launch.</li></ol><p className="mt-2 text-xs">The saved campaign and ad sets remain PAUSED. Retry continues the same hierarchy after the Meta app is Live/Public.</p></div>}
        {['failed','rejected','launch_failed'].includes(job.status)&&<div className="border-t p-5"><button onClick={retryJob} disabled={busy==='retry'} className="inline-flex items-center gap-2 rounded-xl bg-violet-600 px-4 py-2.5 text-sm font-bold text-white disabled:opacity-50">{busy==='retry'?<Loader2 className="h-4 w-4 animate-spin"/>:<Sparkles className="h-4 w-4"/>} {job.status==='launch_failed'?'Retry Meta launch':'Resume from saved checkpoint'}</button><p className="mt-2 text-xs text-slate-500">{job.status==='launch_failed'?(savedMetaDraft?'The approved plan is kept and retry continues the same PAUSED Meta campaign without duplicating it.':'The approved plan is kept; Meta campaign creation is retried after the connection issue is fixed.'):'Completed evidence, copy, and generated images are reused when available.'}</p></div>}
      </Card>}
