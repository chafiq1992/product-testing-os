...
AD_LAUNCHER_META_API_VERSION="v26.0"
AD_LAUNCHER_MODEL="gpt-5.6-sol"
AD_LAUNCHER_COPY_MODEL="gpt-5.6-sol"
AD_LAUNCHER_COPY_REASONING_EFFORT="high"
AD_LAUNCHER_REVIEW_MODEL="gpt-5.6-sol"
AD_LAUNCHER_REVIEW_REASONING_EFFORT="high"
...
# OPENAI_API_KEY, GEMINI_API_KEY, SHOPIFY_ACCESS_TOKEN, META_ACCESS_TOKEN, CLARITY_API_TOKEN

API_FLAGS=(
  --project "$PROJECT_ID" --region "$REGION" --image "$API_IMG" --platform managed
  --allow-unauthenticated --port 8000
  --set-env-vars SHOPIFY_SHOP_DOMAIN="$SHOPIFY_SHOP_DOMAIN",SHOPIFY_PUBLIC_DOMAIN="$SHOPIFY_PUBLIC_DOMAIN",SHOPIFY_API_VERSION="$SHOPIFY_API_VERSION",META_AD_ACCOUNT_ID="$META_AD_ACCOUNT_ID",META_PAGE_ID="$META_PAGE_ID",META_PIXEL_ID="$META_PIXEL_ID",META_API_VERSION="$META_API_VERSION",AD_LAUNCHER_META_API_VERSION="$AD_LAUNCHER_META_API_VERSION",CELERY_BROKER_URL="$CELERY_BROKER_URL",CELERY_RESULT_BACKEND="$CELERY_RESULT_BACKEND",CHATKIT_WORKFLOW_ID="$CHATKIT_WORKFLOW_ID",CHATKIT_WORKFLOW_VERSION="$CHATKIT_WORKFLOW_VERSION",AD_LAUNCHER_MODEL="$AD_LAUNCHER_MODEL",AD_LAUNCHER_COPY_MODEL="$AD_LAUNCHER_COPY_MODEL",AD_LAUNCHER_COPY_REASONING_EFFORT="$AD_LAUNCHER_COPY_REASONING_EFFORT",AD_LAUNCHER_REVIEW_MODEL="$AD_LAUNCHER_REVIEW_MODEL",AD_LAUNCHER_REVIEW_REASONING_EFFORT="$AD_LAUNCHER_REVIEW_REASONING_EFFORT",OPENAI_IMAGE_MODEL="$OPENAI_IMAGE_MODEL",OPENAI_IMAGE_QUALITY="$OPENAI_IMAGE_QUALITY",OPENAI_IMAGE_SIZE="$OPENAI_IMAGE_SIZE"
  --set-secrets OPENAI_API_KEY=OPENAI_API_KEY:latest,GEMINI_API_KEY=GEMINI_API_KEY:latest,SHOPIFY_ACCESS_TOKEN=SHOPIFY_ACCESS_TOKEN:latest,META_ACCESS_TOKEN=META_ACCESS_TOKEN:latest,CLARITY_API_TOKEN=CLARITY_API_TOKEN:latest
