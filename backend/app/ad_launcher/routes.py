...
def _prepare_and_optionally_launch(job_id: str, store: str, auto_launch: bool) -> None:
    service.prepare_job(job_id, store)
    if auto_launch:
...
async def get_connection(request: Request, store: str):
    ...
    return {"data": await run_in_threadpool(service.connection, store)}
